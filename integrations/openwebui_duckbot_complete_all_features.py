...
class Tools:
    ...
    def duckbot_start_ecosystem(self, mode: str = "unified") -> str:
        ...
        if mode not in mode_commands:
            ...
            return f"[FAIL] Invalid mode. Available modes: {available}"
    ...
    def duckbot_emergency_kill(self) -> str:
        ...
        """
        return "[STOP] Emergency killing all DuckBot processes...\n\n" + self.run_batch_command("EMERGENCY_KILL.bat")
    ...
    def duckbot_queue_task(self, task_type: str, prompt: str, priority: str = "low") -> str:
        ...
        if "queued" in result:
            ...
            return f"[FAIL] Failed to queue: {result.get('error', 'Unknown error')}"
    ...
    def duckbot_detect_services(self) -> str:
        ...
        if result.get("ok"):
            ...
            report = "[EMOJI] **Service Detection Results:**\n\n"
            
            for service_name, service_info in services.items():
                status = service_info.get("status", "unknown")
                port = service_info.get("port", "N/A")
                emoji = "[OK]" if status in ["running_healthy", "running_unhealthy"] else "[FAIL]"
    ...
    def duckbot_set_model(self, model_id: str) -> str:
        ...
        if result.get("ok"):
            return f"[OK] Model set to: {result.get('model_set', model_id)}"
        else:
    ...
    def duckbot_refresh_models(self) -> str:
        ...
        if result.get("ok") or result.get("success"):
            return f"[EMOJI] Model cache refreshed: {result.get('message', 'Complete')}"
        else:
            return f"[FAIL] Failed to refresh: {result.get('error', 'Unknown error')}"
    ...
    def duckbot_rag_status(self) -> str:
        ...
        if result.get("ok"):
            ...
            return f"[FAIL] RAG status error: {result.get('error', 'RAG unavailable')}"
    ...
    def duckbot_rag_clear(self) -> str:
        ...
        if result.get("ok"):
            return "[OK] RAG knowledge base cleared"
        else:
    ...
    def duckbot_vibevoice_status(self) -> str:
        ...
        try:
            ...
            if response.status_code == 200:
                data = response.json()
                return f"[OK] **VibeVoice Server Status:**\n\n• Server: Running\n• Available Voices: {len(data.get('voices', []))}\n• URL: http://localhost:8000"
            else:
                return f"[WARN] VibeVoice server responded with status {response.status_code}"
        except Exception as e:
            return f"[FAIL] **VibeVoice Server:** Not accessible\n• Error: {str(e)}\n• Solution: Start server with duckbot_vibevoice_start_server()"
    ...
    def duckbot_qwen_status(self) -> str:
        ...
        if result.get("ok"):
            ...
            return f"[EMOJI] **Qwen Code Analysis:**\n\n• Available: {'[OK]' if available else '[FAIL]'}\n• Integration: {'[OK]' if enabled else '[FAIL]'}\n• Temp Dir: {result.get('temp_dir', 'Unknown')}"
        else:
            return f"[FAIL] Qwen status error: {result.get('error', 'Qwen unavailable')}"
    ...
    def duckbot_action_summary(self, hours: int = 24) -> str:
        ...
        if not result.get("ok"):
            return f"[FAIL] Summary unavailable: {result.get('error', 'Logging offline')}"
        
        summary_data = result.get("summary", {})
        if not summary_data:
            return f"[CHART] No summary data (last {hours} hours)"
    ...
    def duckbot_doctor_mode(self, diagnostic_type: str = "quick") -> str:
        ...
        return f"[EMOJI] **System Doctor ({diagnostic_type}):**\n\n{self.run_batch_command(command)}"
    ...
    def duckbot_start_discord_bot(self, mode: str = "enhanced") -> str:
        ...
        return f"[AI] **Starting Discord bot ({mode}):**\n\n{self.run_batch_command(command)}"
