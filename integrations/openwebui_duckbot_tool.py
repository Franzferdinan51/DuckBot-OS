...
def execute_ai_task(prompt: str, task_type: str = "auto", priority: str = "medium") -> str:
    ...
    if result.get("success"):
        ...
        return f"[AI] {response}\n\n[CHART] Model: {model} | Confidence: {confidence:.2f}{cache_info}"
    
    return f"[FAIL] Failed to execute AI task: {result}"
...
def get_system_status() -> str:
    ...
    status_report = "[LAUNCH] **DuckBot System Status**\n\n"
    
    # AI Router Status
    if ai_status.get("ok"):
        ai_info = ai_status.get("status", {})
        status_report += "[EMOJI] **AI Router:**\n"
    ...
    if services_status.get("ok"):
        ...
        status_report += "[SETTINGS] **Services:**\n"
...
def get_cost_summary(days: int = 7) -> str:
    ...
    if not result.get("success"):
        return f"[FAIL] Could not retrieve cost data: {result.get('error', 'Unknown error')}"
...
def execute_rag_search(query: str, top_k: int = 4) -> str:
    ...
    if not result.get("ok"):
        return f"[FAIL] RAG search failed: {result.get('error', 'Unknown error')}"
    
    context = result.get("context", "")
    chunks = result.get("chunks", [])
    
    if not context:
        return "[EMOJI] No relevant information found in RAG index."
    
    response = f"[EMOJI] **RAG Search Results for: '{query}'**\n\n"
...
def get_available_models() -> str:
    ...
    if not result.get("ok"):
        return f"[FAIL] Could not get models: {result.get('error', 'Unknown error')}"
    
    models = result.get("models", [])
    lm_studio_url = result.get("lm_studio_url", "Unknown")
    
    if not models:
        return f"[EMOJI] No models available from LM Studio ({lm_studio_url})\nMake sure LM Studio is running with at least one model loaded."
    
    response = f"[AI] **Available Models ({len(models)} found)**\n\n"
...
def duckbot_command(
    ...
    try:
        if command == "ai_task":
            if not prompt:
                return "[FAIL] Please provide a prompt for the AI task"
            ...
            if not service_name or not action:
                return "[FAIL] Please provide service_name and action for service management"
            ...
            if not query:
                return "[FAIL] Please provide a query for RAG search"
