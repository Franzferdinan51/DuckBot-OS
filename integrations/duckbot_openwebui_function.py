...
class Tools:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if result.get("success"):
                    ...
                    return f"[AI] **DuckBot AI Response:**\n\n{ai_response}\n\n[CHART] **Details:** Model: {model} | Confidence: {confidence:.2f}"
                else:
                    return f"[FAIL] **AI Error:** {result.get('response', 'Unknown error')}"
            else:
                return f"[FAIL] **HTTP Error:** {response.status_code} - {response.text}"
                
        except requests.exceptions.ConnectionError:
            return "[FAIL] **Connection Error:** Cannot connect to DuckBot server. Is it running at localhost:8787?"
        except requests.exceptions.Timeout:
            return "[FAIL] **Timeout Error:** Request timed out. DuckBot may be busy processing."
    ...
    def duckbot_system_status(self) -> str:
        ...
        try:
            token = self.get_duckbot_token()
            if not token:
                return "[FAIL] **DuckBot Offline:** Server not available at http://localhost:8787"
            ...
            if services_response.status_code == 200:
                ...
                if services_data.get("ok"):
                    ...
                    report += "[SETTINGS] **Services Status:**\n"
                    for svc in services:
                    ...
                    report += f"\n[CHART] **Summary:** {running_count}/{total_count} services running"
                else:
                    report += "[WARN] **Services:** Status unavailable\n"
            else:
                report += "[WARN] **Services:** Cannot retrieve service status\n"
            
            return report
            
        except Exception as e:
            return f"[FAIL] **Error getting system status:** {str(e)}"
    ...
    def duckbot_start_service(self, service_name: str) -> str:
        ...
        if not service_name.strip():
            ...
            return f"[FAIL] **Missing Service Name**\n\nAvailable services: {', '.join(available_services)}"
        
        try:
            token = self.get_duckbot_token()
            if not token:
                return "[FAIL] **DuckBot Offline:** Cannot connect to server"
    ...
    def duckbot_stop_service(self, service_name: str) -> str:
        ...
        if not service_name.strip():
            return "[FAIL] **Missing Service Name:** Please specify which service to stop"
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if result.get("success"):
                    ...
                    return f"[STOP] **Service Stopped**\n\n**{service_name.title()}:** {message}"
                else:
                    error = result.get("error", "Unknown error occurred")
                    return f"[FAIL] **Failed to Stop {service_name.title()}**\n\n**Error:** {error}"
            else:
    ...
    def duckbot_cost_summary(self, days: int = 7) -> str:
        ...
        if days < 1 or days > 365:
            return "[FAIL] **Invalid Days:** Please specify 1-365 days"
        
        try:
            token = self.get_duckbot_token()
            if not token:
                return "[FAIL] **DuckBot Offline:** Cannot retrieve cost data"
            ...
            if response.status_code == 200:
                ...
                if result.get("success"):
                    ...
                    summary += f"[EMOJI] **Total Cost:** ${total_cost:.4f}\n"
                    summary += f"[EMOJI] **Total Tokens:** {total_tokens:,}\n"
                    summary += f"[CHART] **Total Requests:** {total_requests:,}\n\n"
                    
                    # Cost per request
                    if total_requests > 0:
                        cost_per_request = total_cost / total_requests
                        summary += f"[EMOJI] **Avg Cost/Request:** ${cost_per_request:.6f}\n\n"
                    ...
                    return f"[FAIL] **Cost Data Error:** {result.get('error', 'Analytics system unavailable')}"
            else:
                return f"[FAIL] **HTTP Error {response.status_code}:** Cannot retrieve cost data"
    ...
    def duckbot_rag_search(self, query: str, top_k: int = 5) -> str:
        ...
        if not query.strip():
            return "[FAIL] **Missing Query:** Please provide a search query for the knowledge base"
        ...
        try:
            token = self.get_duckbot_token()
            if not token:
                return "[FAIL] **DuckBot Offline:** Cannot access knowledge base"
            ...
            if response.status_code == 200:
                ...
                if result.get("ok"):
                    ...
                    search_result += f"**[DOC] Relevant Content:**\n{context_preview}"
                    if len(context) > 800:
                        search_result += "\n\n*(Content truncated for display)*"
                    
                    # Sources
                    if chunks:
                        search_result += f"\n\n**[DOCS] Sources ({len(chunks)} documents):**\n"
                    ...
                    return f"[FAIL] **RAG Search Failed:** {error_msg}"
            else:
                return f"[FAIL] **HTTP Error {response.status_code}:** Cannot access knowledge base"
                
        except Exception as e:
            return f"[FAIL] **Error searching knowledge base:** {str(e)}"
    ...
    def duckbot_list_models(self) -> str:
        ...
        try:
            token = self.get_duckbot_token()
            if not token:
                return "[FAIL] **DuckBot Offline:** Cannot retrieve model information"
            ...
            if response.status_code == 200:
                ...
                if result.get("ok"):
                    ...
                    model_list = f"[AI] **Available AI Models**\n\n"
                    ...
                    model_list += "[EMOJI] **Tip:** Use `duckbot_ai_chat()` to interact with the active model"
                    
                    return model_list
                else:
                    error_msg = result.get("error", "LM Studio connection failed")
                    return f"[FAIL] **Models Unavailable:** {error_msg}\n\n**Check:**\n• LM Studio is running\n• Local server is enabled\n• At least one model is loaded"
            else:
                return f"[FAIL] **HTTP Error {response.status_code}:** Cannot retrieve model information"
