...
// DuckBot Backend Integration - Use local DuckBot APIs for optimal integration
async function getDuckBotResponse(prompt: string): Promise<string> {
    ...
    try {
        ...
        console.error("Error calling DuckBot API:", error);
        throw new Error("Failed to fetch response from DuckBot.");
    }
}

// Original OpenRouter integration - Keep for fallback/alternative use
...
const App: React.FC = () => {
    // Enhanced State Management for Complete DuckBot OS
    const [isLoading, setIsLoading] = useState(false);
    const [isModelLoaded, setIsModelLoaded] = useState(false);
    const [loadProgress, setLoadProgress] = useState(0);
    const [morphTargetDictionary, setMorphTargetDictionary] = useState<MorphTargetDictionary | null>(null);
    const [modelUrl, setModelUrl] = useState<string>('../Blender.glb');
    const [currentObjectUrl, setCurrentObjectUrl] = useState<string | null>(null);

    // DuckBot OS Desktop Environment State
    const [isDuckBotOSMode, setIsDuckBotOSMode] = useState(false);
    ...
    // Complete AI System State
    const [isSettingsOpen, setIsSettingsOpen] = useState(false);
    const [settings, setSettings] = useState<Settings>({
        apiProvider: 'duckbot',
        geminiModel: GEMINI_MODELS[0],
        openRouterApiKey: '',
        openRouterModel: 'auto-selected',
        speechVoiceURI: null,
    });
    ...
    const [currentModel, setCurrentModel] = useState('duckbot/auto');
    const [agentStatus, setAgentStatus] = useState('idle');
    const [conversation, setConversation] = useState([]);
    const [isAgentActive, setIsAgentActive] = useState(true);
    const [systemHealth, setSystemHealth] = useState({ cpu: 23, memory: 45, gpu: 12, temperature: 58 });
    
    // File System and Applications
    const [files, setFiles] = useState([
        { name: 'README.md', type: 'markdown', size: '1.2 KB', modified: 'Today, 10:30 AM', content: '# Welcome to DuckBot OS\n\nComplete AI Operating System with ALL features.' },
        { name: 'duckbot.py', type: 'python', size: '5.2 KB', modified: 'Yesterday, 3:45 PM', content: '# DuckBot Enhanced System\nprint("🦆 DuckBot OS - Complete AI Experience")\n\nfor feature in ["AI Models", "3D Avatar", "Voice", "Desktop"]:\n    print(f"✅ {feature} loaded successfully")' },
        { name: 'startup.js', type: 'javascript', size: '3.8 KB', modified: 'Yesterday, 2:30 PM', content: '// DuckBot OS Startup Script\nconsole.log("🦆 DuckBot OS initializing...");\n\nfunction initializeAllFeatures() {\n    return "All DuckBot OS features loaded!";\n}\n\nconsole.log(initializeAllFeatures());' },
    ]);
    
    // Browser and Web Integration
    const [currentUrl, setCurrentUrl] = useState('home.duckbot-os.local');
    const [browserHistory, setBrowserHistory] = useState(['home.duckbot-os.local']);
    
    // DuckBot Helper State
    const [duckBotPosition, setDuckBotPosition] = useState({ x: 20, y: 20 });
    const [isDuckBotVisible, setIsDuckBotVisible] = useState(true);
    const [duckBotMessage, setDuckBotMessage] = useState('');
    const [isDuckBotSpeaking, setIsDuckBotSpeaking] = useState(false);
    ...
    const speakAndAnimate = useCallback((text: string) => {
        ...
        if (morphTargetDictionary) {
            ...
            if (!mouthMorphTargetName) {
                ...
                mouthMorphTargetName = allKeys.find(key => key.toLowerCase().includes('mouth') || key.toLowerCase().includes('jaw') || key.toLowerCase().includes('open')) || null;
                if (mouthMorphTargetName) {
    ...
    const handleSend = async (message: string) => {
        ...
        try {
            ...
            } else if (settings.apiProvider === 'duckbot') {
                responseText = await getDuckBotResponse(message);
            } else {
                // Default to DuckBot for best integration
                responseText = await getDuckBotResponse(message);
            }
    ...
    return (
        <div className="relative w-screen h-screen overflow-hidden bg-gray-800">
            <ThreeScene ref={sceneRef} modelUrl={modelUrl} onModelLoad={handleModelLoad} onLoadProgress={handleLoadProgress} />
                    ...
                    <div className="bg-gray-800 rounded-lg shadow-xl p-6 w-full max-w-md text-white border border-gray-700" onClick={(e) => e.stopPropagation()}>
                        ...
                        <div className="space-y-6">
                            <div>
                                ...
                                <p className="text-xs text-gray-400 mt-2">Supported: GLB, GLTF, VRM, FBX. Note: .BLEND files are not supported directly.</p>
                            </div>
                            ...
                            <div>
                                ...
                                <div className="flex gap-2 p-1 bg-gray-700 rounded-md">
                                    <button onClick={() => setTempSettings({...tempSettings, apiProvider: 'duckbot'})} className={`flex-1 py-1 rounded text-sm ${tempSettings.apiProvider === 'duckbot' ? 'bg-green-600' : 'hover:bg-gray-600'}`}>DuckBot</button>
                                    <button onClick={() => setTempSettings({...tempSettings, apiProvider: 'gemini'})} className={`flex-1 py-1 rounded text-sm ${tempSettings.apiProvider === 'gemini' ? 'bg-blue-600' : 'hover:bg-gray-600'}`}>Gemini</button>
                                    <button onClick={() => setTempSettings({...tempSettings, apiProvider: 'openrouter'})} className={`flex-1 py-1 rounded text-sm ${tempSettings.apiProvider === 'openrouter' ? 'bg-purple-600' : 'hover:bg-gray-600'}`}>OpenRouter</button>
                                </div>
                            </div>
                            ...
                            {tempSettings.apiProvider === 'duckbot' ? (
                                <div className="p-4 bg-gradient-to-r from-green-900 to-green-800 rounded-md border border-green-600">
                                    <h3 className="text-lg font-bold text-green-200 mb-2">🦆 DuckBot Enhanced Integration</h3>
                                    <p className="text-sm text-green-100 mb-2">Connected to your local DuckBot system with intelligent model routing:</p>
                                    <ul className="text-xs text-green-200 space-y-1">
                                        <li>• Auto-selects best available model (LM Studio → OpenRouter → Qwen)</li>
                                        <li>• Uses Windows device voices (VibeVoice fallback disabled)</li>
                                        <li>• Integrated with DuckBot's dynamic model manager</li>
                                        <li>• Optimized for your RTX 3080 hardware configuration</li>
                                    </ul>
                                </div>
