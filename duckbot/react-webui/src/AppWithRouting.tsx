...
import { ThreeScene } from './components/components/ThreeScene.tsx';
import { ChatUI } from './components/components/ChatUI.tsx';
import GitHubRepositoryManager from './components/GitHubRepositoryManager.tsx';
import EnhancedSystemDashboard from './components/EnhancedSystemDashboard.js';
import { AIService } from './services/duckbotService.ts';
import { githubService } from './services/githubService.ts';
import DeepCodeDashboard from './components/applications/DeepCodeDashboard.js';
import DeepCodePaper2Code from './components/applications/DeepCodePaper2Code.js';
import DeepCodeText2Web from './components/applications/DeepCodeText2Web.js';
import DeepCodeText2Backend from './components/applications/DeepCodeText2Backend.js';
import DeepCodeConfig from './components/applications/DeepCodeConfig.js';
import {
  Home,
  GitBranch,
  Settings,
...
import RAGRouter from './components/applications/RAGRouter.js';

// Navigation component
const Navigation: React.FC<{ currentPath: string; onNavigate: (path: string) => void }> = ({ currentPath, onNavigate }) => {
  const navItems = [
    { path: '/', icon: Home, label: 'Home', description: '3D Assistant' },
    { path: '/dashboard', icon: Monitor, label: 'Dashboard', description: 'System Management' },
    { path: '/services', icon: Zap, label: 'Services', description: 'Service Control' },
    { path: '/network', icon: Network, label: 'Network', description: 'Port Management' },
    { path: '/github', icon: GitBranch, label: 'GitHub', description: 'Repository Manager' },
    { path: '/deepcode', icon: Brain, label: 'DeepCode', description: 'AI Code Generation' },
    { path: '/rag', icon: Search, label: 'RAG', description: 'Knowledge Search' },
  ];

  return (
    <nav className="absolute top-4 left-4 z-20">
      <div className="bg-gray-800 rounded-lg border border-gray-700 p-2 space-y-1">
        {navItems.map(item => {
...
const AppWithRouting: React.FC = () => {
  ...
  // Initialize GitHub WebSocket connection
  useEffect(() => {
    if (settings.githubToken && currentPath === '/github') {
      githubService.connectWebSocket();
    }

    return () => {
      if (currentPath !== '/github') {
        githubService.disconnectWebSocket();
      }
    };
  }, [settings.githubToken, currentPath]);

  // Listen for system status updates from Electron main process
  useEffect(() => {
    if (window.electronAPI) {
      const handleSystemStatusUpdate = (event, data) => {
        console.log('System status updated:', data);
        // Could update local state here if needed
      };

      window.electronAPI.onSystemStatusUpdated(handleSystemStatusUpdate);

      return () => {
        if (window.electronAPI?.removeSystemStatusListener) {
          window.electronAPI.removeSystemStatusListener(handleSystemStatusUpdate);
        }
      };
    }
  }, []);

  const handleModelLoad = useCallback((dictionary) => {
  ...
  const handleSaveSettings = () => {
    ...
    try {
      ...
      if (tempSettings.githubToken !== settings.githubToken) {
        ...
        if (tempSettings.githubToken && currentPath === '/github') {
          githubService.connectWebSocket();
        }
      }
    } catch (error) {
  ...
  return (
    <div className="relative w-screen h-screen overflow-hidden bg-gradient-to-br from-gray-900 via-gray-800 to-gray-900">
      {/* 3D Scene - only show on home page */}
      {currentPath === '/' && (
        <ThreeScene ref={sceneRef} onModelLoad={handleModelLoad} onLoadProgress={handleLoadProgress} />
      )}

      {/* Loading overlay - only show on home page */}
      {currentPath === '/' && !isModelLoaded && (
      ...
      {/* Settings Modal */}
      {isSettingsOpen && (
        <div className="absolute inset-0 z-50 flex items-center justify-center bg-black bg-opacity-80" onClick={() => setIsSettingsOpen(false)}>
          <div className="bg-gray-800 rounded-lg shadow-xl p-6 w-full max-w-md text-white border border-gray-700" onClick={(e) => e.stopPropagation()}>
            <h2 className="text-2xl font-bold mb-6">DuckBot Settings</h2>
        ...
        <Routes>
          ...
          <Route path="/github" element={
            <GitHubRepositoryManager
              baseUrl={settings.duckbotUrl}
              token={settings.githubToken}
              className="w-full h-full"
            />
          } />

          <Route path="/deepcode" element={
            <div className="w-full h-full">
              <DeepCodeDashboard />
            </div>
          } />

          <Route path="/deepcode/paper2code" element={
            <div className="w-full h-full">
              <DeepCodePaper2Code />
            </div>
          } />

          <Route path="/deepcode/text2web" element={
            <div className="w-full h-full">
              <DeepCodeText2Web />
            </div>
          } />

          <Route path="/deepcode/text2backend" element={
            <div className="w-full h-full">
              <DeepCodeText2Backend />
            </div>
          } />

          <Route path="/deepcode/config" element={
            <div className="w-full h-full">
              <DeepCodeConfig />
            </div>
          } />
