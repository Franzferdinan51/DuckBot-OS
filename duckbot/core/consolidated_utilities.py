...
class DuckBotConsolidatedUtilities:
    ...
    def check_integration_availability(self, integration_name: str) -> Dict[str, Any]:
        """Check if a specific integration is available"""
        integration_checks = {
            "lm_studio": self._check_lm_studio_availability,
            "comfyui": self._check_comfyui_availability,
            "webui": self._check_webui_availability,
            "discord": self._check_discord_availability,
            "vibevoice": self._check_vibevoice_availability,
            "archon": self._check_archon_availability,
            "qwen_agent": self._check_qwen_agent_availability,
            "mcp_server": self._check_mcp_server_availability
        }
        
        if integration_name in integration_checks:
            return integration_checks[integration_name]()
        else:
            return {"available": False, "error": f"Unknown integration: {integration_name}"}

    def _check_lm_studio_availability(self) -> Dict[str, Any]:
    ...
    def _check_comfyui_availability(self) -> Dict[str, Any]:
        """Check ComfyUI availability"""
        try:
            import requests
            response = requests.get("http://localhost:8188/object_info", timeout=5)
            if response.status_code == 200:
                return {"available": True, "version": "ComfyUI"}
            else:
                return {"available": False, "error": f"Status {response.status_code}"}
        except Exception as e:
            return {"available": False, "error": str(e)}

    def _check_webui_availability(self) -> Dict[str, Any]:
    ...
    def _check_discord_availability(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if token and token != "your_discord_token_here":
                ...
                return {"available": True, "configured": False, "error": "Token not set"}
        except Exception as e:
            return {"available": False, "error": str(e)}

    def _check_vibevoice_availability(self) -> Dict[str, Any]:
        """Check VibeVoice availability"""
        try:
            # Check if VibeVoice files exist
            vibevoice_files = [
                "duckbot/vibevoice_client.py",
                "duckbot/vibevoice_commands.py"
            ]
            
            available_files = [f for f in vibevoice_files if Path(f).exists()]
            return {
                "available": len(available_files) > 0,
                "files_found": available_files,
                "files_missing": [f for f in vibevoice_files if f not in available_files]
            }
        except Exception as e:
            return {"available": False, "error": str(e)}

    def _check_archon_availability(self) -> Dict[str, Any]:
        """Check Archon availability"""
        try:
            # Check if Archon files exist
            archon_files = [
                "duckbot/archon_integration.py",
                "duckbot/metagpt_archon_integration.py"
            ]
            
            available_files = [f for f in archon_files if Path(f).exists()]
            return {
                "available": len(available_files) > 0,
                "files_found": available_files,
                "files_missing": [f for f in archon_files if f not in available_files]
            }
        except Exception as e:
            return {"available": False, "error": str(e)}

    def _check_qwen_agent_availability(self) -> Dict[str, Any]:
        """Check Qwen-Agent availability"""
        try:
            import importlib
            qwen_module = importlib.import_module("duckbot.qwen_agent_integration")
            return {"available": True, "module": "duckbot.qwen_agent_integration"}
        except ImportError:
            return {"available": False, "error": "Module not found"}
        except Exception as e:
            return {"available": False, "error": str(e)}

    def _check_mcp_server_availability(self) -> Dict[str, Any]:
        """Check MCP Server availability"""
        try:
            import importlib
            mcp_module = importlib.import_module("duckbot.integrations.mcp_server")
            return {"available": True, "module": "duckbot.integrations.mcp_server"}
        except ImportError:
            return {"available": False, "error": "Module not found"}
        except Exception as e:
            return {"available": False, "error": str(e)}
    ...
    def run_comprehensive_tests(self) -> Dict[str, Any]:
        ...
        # Test categories
        test_categories = {
            "core_system": self._test_core_system,
            "ai_routing": self._test_ai_routing,
            "integrations": self._test_integrations,
            "webui": self._test_webui,
    ...
    def _test_core_system(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # Calculate score
            passed_imports = sum(1 for result in import_results.values() if result)
            total_imports = len(import_results)
            score = (passed_imports / total_imports * 100) if total_imports > 0 else 0
    ...
    def _test_ai_routing(self) -> Dict[str, Any]:
        """Test AI routing functionality"""
        try:
            # Test AI provider setup
            lm_studio_result = self.setup_ai_provider("lm_studio")
            openrouter_result = self.setup_ai_provider("openrouter")
            
            # Test model availability
            lm_models = self.get_available_ai_models("lm_studio")
            or_models = self.get_available_ai_models("openrouter")
            
            # Test connections
            lm_conn = self.test_ai_provider_connection("lm_studio")
            or_conn = self.test_ai_provider_connection("openrouter")
            
            return {
                "success": lm_studio_result["success"] or openrouter_result["success"],
                "providers": {
                    "lm_studio": {
                        "setup": lm_studio_result["success"],
                        "models": lm_models,
                        "connection": lm_conn["success"]
                    },
                    "openrouter": {
                        "setup": openrouter_result["success"],
                        "models": or_models,
                        "connection": or_conn["success"]
                    }
                }
            }
        except Exception as e:
    ...
    def _test_integrations(self) -> Dict[str, Any]:
        ...
        try:
            integrations = [
                "lm_studio", "comfyui", "webui", "discord",
                "vibevoice", "archon", "qwen_agent", "mcp_server"
            ]
            
            results = {}
            for integration in integrations:
                results[integration] = self.check_integration_availability(integration)
            
            # Calculate availability
            available_count = sum(1 for r in results.values() if r.get("available", False))
            total_count = len(results)
            score = (available_count / total_count * 100) if total_count > 0 else 0
    ...
    def _test_webui(self) -> Dict[str, Any]:
        """Test WebUI functionality"""
        try:
            # Test WebUI availability
            webui_status = self._check_webui_availability()
            
            # Test key routes (if WebUI is available)
            routes_tested = []
            if webui_status.get("available"):
                try:
                    import requests
                    base_url = "http://localhost:8787"
                    routes = ["/healthz", "/api/status", "/api/models"]
            ...
            return {
                "success": webui_status.get("available", False),
    ...
    def _test_system_resources(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                "success": resource_score >= 20,  # At least 20% free resources
    ...
    def _test_configuration(self) -> Dict[str, Any]:
        ...
        try:
            ...
            config_score = (found_count / total_configs * 100) if total_configs > 0 else 0
            
            return {
                "success": config_score >= 50,  # 50% threshold
...
def run_tests() -> Dict[str, Any]:
    """Run comprehensive system tests"""
    return utilities.run_comprehensive_tests()

def check_integration(integration_name: str) -> Dict[str, Any]:
    """Check integration availability"""
    return utilities.check_integration_availability(integration_name)
...
def main():
    ...
    parser.add_argument("command", choices=[
        ...
        "system-status", "python-info", "merge-configs", "run-tests",
        "check-integration"
    ...
    parser.add_argument("--integration", "-i", help="Integration to check")
    ...
    try:
        if args.command == "backup":
            ...
            if args.integration:
                result = check_integration(args.integration)
                print(json.dumps(result, indent=2))
            else:
                print("Error: --integration required for check-integration")
                return 1
