...
def main():
    ...
    print("[EMOJI] DuckBot Hardware Detection")
    print("=" * 50)
    
    detector = HardwareDetector()
    config = detector.detect_all_hardware()
    
    print("\n[CHART] Hardware Detection Results:")
    print(json.dumps(config, indent=2))
    
    # Save to file
    config_file = detector.save_hardware_config()
    if config_file:
        print(f"\n[SAVE] Configuration saved to: {config_file}")
    
    print("\n[TARGET] Performance Tier:", config["performance_tier"])
    print("[EMOJI] Recommended Models:", ", ".join(config["model_recommendations"]["main_brain_candidates"]))
