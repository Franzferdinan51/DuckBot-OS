...
def get_notebook_status() -> str:
    ...
    if client.is_available():
        return f"[OK] **Open Notebook Active**\n[EMOJI] Web Interface: {client.web_url}\n[EMOJI] API Backend: {client.api_url}"
    else:
        return "[FAIL] **Open Notebook Offline**\nUse `start_open_notebook.bat` to start services"
...
async def handle_notebook_command(title: str, description: str = "") -> str:
    ...
    if not client.is_available():
        return "[FAIL] Open Notebook services are not running. Please start them first."
    
    result = await client.create_notebook(title, description)
    
    if result["success"]:
        return f"[OK] **Notebook Created!**\n[EMOJI] Title: {title}\n[EMOJI] Access: {client.web_url}"
    else:
        return f"[FAIL] **Failed to create notebook**\nError: {result['error']}"
