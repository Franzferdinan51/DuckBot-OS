                ...
                if (isImageRequest) {
                    ...
                    try {
                        ...
                        if (services.error) {
                            ...
                            if (servicesList.length > 0) {
                                const statusReport = servicesList.map((svc: any) => `${svc.name}: ${svc.status}`).join(', ');
                                responseText = `Service Status: ${statusReport}. All systems are ${servicesList.every((svc: any) => svc.status === 'running') ? 'operational' : 'mixed'}.`;
                            } else {
                    ...
                    try {
                        ...
                        if (ragResults.error) {
                            ...
                            if (results.length > 0) {
                                responseText = `Found ${results.length} relevant documents: ${results.map((r: any) => r.title || r.content?.substring(0, 100)).join(', ')}`;
                            } else {
