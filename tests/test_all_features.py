...
def test_service_detection():
    ...
    try:
        ...
        print(f"[FAIL] Service detection failed: {e}")
        return False
...
def test_server_management():
    ...
    try:
        ...
        print(f"[FAIL] Server management failed: {e}")
        return False
...
def test_ai_routing():
    ...
    try:
        ...
        print(f"[FAIL] AI routing failed: {e}")
        return False
    ...
    try:
        ...
        for test_name, endpoint, method, test_headers in tests:
            try:
                ...
                print(f"  [OK] {test_name}: {response.status_code}")
        ...
        print(f"[FAIL] WebUI test failed: {e}")
        return False
...
def main():
    ...
    try:
        print(f"[STATS] Test Results: {passed_tests}/{total_tests} categories passed")
    ...
    for category, result in test_results.items():
        if isinstance(result, dict):
            ...
            try:
                status = "[OK]" if sub_passed == sub_total else "[WARN]" if sub_passed > 0 else "[FAIL]"
            except UnicodeEncodeError:
    ...
    try:
        if passed_tests == total_tests:
            ...
            print(f"\n[WARN]  MOSTLY WORKING - {passed_tests}/{total_tests} categories passed")
            return 1
        else:
            print(f"\n[FAIL] NEEDS ATTENTION - Only {passed_tests}/{total_tests} categories passed")
            return 2
    except UnicodeEncodeError:
        if passed_tests == total_tests:
            print(f"\n[SUCCESS] ALL TESTS PASSED - DuckBot is ready for production!")
