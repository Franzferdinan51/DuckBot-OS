...
REM Pre-flight checks with detailed feedback
echo [PREFLIGHT] Running system checks...
call :check_python_ultimate
if %errorlevel% neq 0 (
    echo [ERROR] Python check failed - returning to main menu
    goto main_menu
)

call :check_system_requirements
if %errorlevel% neq 0 (
    echo [WARNING] System requirements check had issues - continuing anyway
)

call :check_required_files
if %errorlevel% neq 0 (
    echo [ERROR] Required files missing - returning to main menu
    goto main_menu
)
...
echo.
echo [BATCH] Executing: python start_ecosystem.py
echo [BATCH] The console will show live logs and startup messages
echo [BATCH] Once DuckBot starts, it will be available at: http://127.0.0.1:8787
echo [BATCH] Press Ctrl+C in the Python process to stop DuckBot
echo [BATCH] Keep this console window open to see activity
echo [BATCH] ============================================================
echo.
...
echo ================================================================================
echo.
echo [BATCH] Python exit code: %PYTHON_EXIT_CODE%
if %PYTHON_EXIT_CODE% equ 0 (
    echo [BATCH] Status: Normal exit
) else (
    echo [BATCH] Status: Error exit - check output above for details
)
echo [BATCH] The main ecosystem process has finished. 
echo [BATCH] If this was unexpected, please check the console output above for errors.
echo [BATCH] All logs are also saved to the logs/ directory
echo.
...
echo.
Electron Ultimate Desktop Edition session ended.
...
echo [BATCH] Executing: python -m duckbot.enhanced_webui --host 127.0.0.1 --port 8787
echo [BATCH] Web interface will be available at: http://127.0.0.1:8787
echo [BATCH] Press Ctrl+C to stop the WebUI server
echo.
...
echo.
echo [BATCH] Enhanced WebUI session ended with exit code: %WEBUI_EXIT_CODE%
echo [BATCH] All logs saved to logs/ directory
echo.
pause
goto main_menu
...
echo [BATCH] Executing: python -m duckbot.charm_terminal_ui
echo [BATCH] This will start the interactive terminal interface
echo [BATCH] Press Ctrl+C or use the quit option to exit
echo.
...
echo.
echo [BATCH] Charm Terminal session ended with exit code: %CHARM_EXIT_CODE%
pause
goto main_menu
