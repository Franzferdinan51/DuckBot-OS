...
class LearningSystem:
    ...
    def _generate_learning_insights(self, events: List[LearningEvent], 
        ...
        if len(events) > 100:
            ...
            if avg_success > 0.8:
                insights.append("[SUCCESS] System is performing excellently with high success rates")
            elif avg_success > 0.6:
                insights.append("[OK] System performance is good with room for improvement")
            else:
                insights.append("[WARN] System performance needs attention - low success rates detected")
        ...
        for agent_type, trend_data in trends.items():
            if "improvement_rate" in trend_data:
                ...
                if improvement > 0.01:
                    insights.append(f"[EMOJI] {agent_type} agent is learning rapidly (improvement: {improvement:.1%})")
                elif improvement < -0.01:
                    insights.append(f"[EMOJI] {agent_type} agent performance is declining - investigate needed")
        
        # Pattern insights
        if len(patterns) > 0:
            high_conf_patterns = [p for p in patterns if p.get("confidence", 0) > 0.8]
            if high_conf_patterns:
                insights.append(f"[EMOJI] Discovered {len(high_conf_patterns)} high-confidence learning patterns")
