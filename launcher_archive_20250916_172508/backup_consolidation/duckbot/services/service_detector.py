...
class ServiceDetector:
    ...
    # Back-compat helper used by tests: quick boolean service check
    def _is_service_running(self, service_name: str) -> bool:
        """Return True if a known service appears active by any signal.
        Considers health checks, open ports, or matching processes as running.
        """
        try:
            info = self.detect_service_status(service_name)
            if not isinstance(info, dict):
                return False
            status = str(info.get('status') or '')
            if status in ('running_healthy', 'running_unhealthy', 'process_detected'):
                return True
            ports = info.get('ports_in_use') or []
            return bool(ports)
        except Exception:
            return False
