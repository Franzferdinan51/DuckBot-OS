...
REM - Auto-detects local DuckBot-DE folder or a DuckBot-OS*\DuckBot-DE on Desktop.
REM ------------------------------------------------------------------------------
if not defined DUCKBOT_DE_PATH (
    if exist "%CD%\DuckBot-DE" (
        set "DUCKBOT_DE_PATH=%CD%\DuckBot-DE"
    ) else if exist "C:\Users\Ryan\Desktop\DuckBot-OS-v4.1-Complete-20250907_121545\DuckBot-DE" (
        set "DUCKBOT_DE_PATH=C:\Users\Ryan\Desktop\DuckBot-OS-v4.1-Complete-20250907_121545\DuckBot-DE"
    ) else (
        for /f "delims=" %%D in ('dir /s /b /ad "C:\Users\Ryan\Desktop\DuckBot-OS*\DuckBot-DE" 2^>nul ^| sort') do (
            if not defined DUCKBOT_DE_PATH set "DUCKBOT_DE_PATH=%%D"
        )
    )
)
...
start "DuckBot WebUI" python -m duckbot.webui_enhanced --host 127.0.0.1 --port 8787 > logs\enhanced_webui.log 2>&1
timeout /t 2 >nul
netstat -ano | findstr :8787 | findstr LISTENING >nul
if %errorlevel% equ 0 (
...
start "Enhanced WebUI" python -m duckbot.webui_enhanced --host 0.0.0.0 --port 8787
timeout /t 2 >nul
...
python -m duckbot.webui_enhanced --host 127.0.0.1 --port 8787
...
start "Enhanced WebUI" /MIN python -m duckbot.webui_enhanced --host 127.0.0.1 --port 8787
timeout /t 2 >nul
