...
def test_duckbot_os_file():
    print("[EMOJI] Testing DuckBot OS Interface")
    print("=" * 50)
    ...
    for i, path in enumerate(possible_paths, 1):
        ...
        print(f"   Exists: {'[OK] YES' if exists else '[FAIL] NO'}")
        if exists:
    ...
    if found_path:
        print(f"[OK] DuckBot OS file found at: {found_path}")
        ...
        try:
            ...
            print("\n[EMOJI] Component Check:")
            for name, check in checks:
                status = "[OK]" if check else "[FAIL]"
                print(f"   {status} {name}")
            
            all_good = all(check for _, check in checks)
            print(f"\n{'[OK] All components present!' if all_good else '[WARN] Some components missing'}")
            
        except Exception as e:
            print(f"[FAIL] Error reading file: {e}")
            return False
    else:
        print("[FAIL] DuckBot OS file not found!")
        return False
    
    return found_path is not None

def test_webui_routing():
    print("\n[TOOLS] Testing WebUI Routing")
    print("=" * 50)
    
    webui_path = os.path.join("duckbot", "webui.py")
    if not os.path.exists(webui_path):
        print(f"[FAIL] WebUI file not found: {webui_path}")
        return False
    ...
    try:
        ...
        print("[EMOJI] WebUI Route Check:")
        for name, check in checks:
            status = "[OK]" if check else "[FAIL]"
            print(f"   {status} {name}")
            
        all_good = all(check for _, check in checks)
        print(f"\n{'[OK] WebUI routing updated correctly!' if all_good else '[WARN] WebUI routing needs fixing'}")
        return all_good
        
    except Exception as e:
        print(f"[FAIL] Error reading WebUI file: {e}")
        return False

if __name__ == "__main__":
    print("[EMOJI] DuckBot OS Integration Test")
    print("=" * 60)
