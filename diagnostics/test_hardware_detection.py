...
def test_hardware_detection():
    ...
    print("[EMOJI] Testing DuckBot Hardware Detection")
    print("=" * 50)
    
    try:
        from duckbot.hardware_detector import HardwareDetector, detect_hardware
        
        print("[OK] Hardware detector imported successfully")
        
        # Test detection
        print("\n[EMOJI] Running hardware detection...")
...
def test_dynamic_model_manager():
    ...
    print("\n[AI] Testing Dynamic Model Manager")
    print("=" * 50)
    
    try:
        from duckbot.dynamic_model_manager import DynamicModelManager
        
        print("[OK] Dynamic model manager imported successfully")
        
        # Initialize (this will trigger hardware detection)
        print("\n[EMOJI] Initializing with hardware detection...")
        ...
        print(f"\n[TARGET] Hardware-Optimized Models:")
...
def simulate_different_systems():
    ...
    for scenario in test_scenarios:
        ...
        try:
            ...
            print(f"   [TARGET] Performance Tier: {tier}")
            ...
            print(f"   [EMOJI] Recommended Models: {', '.join(models[:2])}")
...
def main():
    ...
    print("\n" + "=" * 50)
    if success:
        print("[OK] All hardware detection tests passed!")
        print("\n[EMOJI] Universal Hardware Optimization Features:")
        print("   [TOOLS] Works on ANY hardware (RTX 4090 to integrated graphics)")
        print("   [SETTINGS]  Auto-detects VRAM, RAM, CPU specs")
        print("   [TARGET] Recommends optimal models for each system")
        print("   [CHART] 6 performance tiers: enterprise → ultra_low")
        print("   [LAUNCH] Automatically optimizes concurrent model limits")
    else:
        print("[FAIL] Some tests failed - check error messages above")
        print("\n[TOOLS] You may need to install missing dependencies:")
        print("   pip install psutil")
    
    print("\n[EMOJI] DuckBot will now work optimally on ANY system!")
